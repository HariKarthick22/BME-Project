...
// Safe split helper — never returns undefined
const safeSplit = (str, sep, idx) => {
    const parts = (str || '').split(sep);
    return parts[idx]?.trim() || parts[0]?.trim() || '—';
};

const generateFallbackImage = (name) => {
    const clean = (name || '').replace(/&/g, 'and');
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="600" height="400"><rect width="100%" height="100%" fill="#e3e2e1"/><text x="50%" y="50%" font-family="sans-serif" font-weight="bold" font-size="24" fill="#006d36" text-anchor="middle" dominant-baseline="middle">${clean}</text></svg>`;
...
export const HospitalDetailPage = () => {
    ...
    const [visaToast, setVisaToast] = useState(false);

    const hospital = location.state?.hospital;

    useEffect(() => {
        if (!hospital) navigate('/');
    }, [hospital, navigate]);

    if (!hospital) return null;

    const formatCurrency = (val) => val ? `₹${(val / 100000).toFixed(1)}L` : 'TBD';

    const tech0    = safeSplit(hospital.technology_stack, ',', 0);
    const doctor0  = safeSplit(hospital.lead_doctors, ',', 0);
    const stay0    = safeSplit(hospital.avg_length_of_stay, ',', 0);
    const stay1    = safeSplit(hospital.avg_length_of_stay, ',', 1);
    const techFull = hospital.technology_stack || tech0;
            ...
            <main className="pt-32 pb-24">
                ...
                <section className="px-12 max-w-screen-2xl mx-auto mb-24">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div className="bg-surface-container-lowest p-12 rounded-xl shadow-[0_12px_32px_rgba(26,28,28,0.06)] flex flex-col justify-between h-64">
                            ...
                            <div>
                                <div className="font-headline text-3xl text-primary mb-2 leading-tight">{tech0}</div>
                        ...
                        <div className="bg-surface-container-lowest p-12 rounded-xl shadow-[0_12px_32px_rgba(26,28,28,0.06)] flex flex-col justify-between h-64">
                            ...
                            <div>
                                <div className="font-headline text-4xl text-primary mb-2">{doctor0}</div>
                                <div className="text-on-surface-variant uppercase tracking-widest text-xs">Lead Surgeon ({hospital.lead_doctor_experience || '15+ years'})</div>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section className="px-12 max-w-screen-2xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16">
                        ...
                        <div className="space-y-0 border-l border-outline-variant/30 ml-4">
                            ...
                            <div className="relative pl-12 pb-12">
                                ...
                                <div className="bg-surface-container-low p-8 rounded-xl">
                                    ...
                                    <p className="text-primary font-medium mb-4">Minimally invasive procedure utilizing {tech0}.</p>
                            ...
                            <div className="relative pl-12 pb-12">
                                <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-stone-300"></div>
                                <span className="font-headline text-2xl text-primary block mb-2">Recovery: {stay0}</span>
                                <p className="text-on-surface-variant text-sm leading-relaxed">Personalized physiotherapy begins under the care of {doctor0}'s team.</p>
                            </div>
                            <div className="relative pl-12">
                                <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-stone-300"></div>
                                <span className="font-headline text-2xl text-primary block mb-2">Therapeutic Rest: {stay1}</span>
                    ...
                    <div className="lg:col-span-5">
                        <div className="bg-surface-container-high rounded-2xl p-10 sticky top-32 shadow-sm">
                            ...
                            <button
                                onClick={() => { setVisaToast(true); setTimeout(() => setVisaToast(false), 3500); }}
                                className="w-full bg-primary text-on-primary py-5 rounded-lg font-bold tracking-[0.1em] text-xs uppercase hover:opacity-90 transition-all flex items-center justify-center gap-2"
                            >
                            ...
                            {visaToast && (
                                <div className="mt-3 bg-secondary-container text-on-secondary-container px-4 py-3 rounded-lg text-xs flex items-center gap-2 animate-pulse">
                                    <span className="material-symbols-outlined text-[16px]">check_circle</span>
                                    Visa invitation letter generated — check your email within 24 hours.
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <section className="mt-32 px-12 max-w-screen-2xl mx-auto">
                    <div className="relative h-[600px] rounded-3xl overflow-hidden group">
                       <img 
    alt={`${hospital.name} Interior`} 
    className="w-full h-full object-cover grayscale-[0.2] transition-transform duration-700 group-hover:scale-105 bg-surface-variant" 
    src={hospital.image_url}
    onError={(e) => {
        e.currentTarget.onerror = null; 
        e.currentTarget.src = generateFallbackImage(hospital.name);
    }}
/>
                        ...
                        <div className="absolute bottom-16 left-16 max-w-2xl">
                            ...
                            <p className="text-on-primary/80 text-lg leading-relaxed mb-8">
                                "In medicine, the difference between a good outcome and a perfect one is measured in millimeters. With our {tech0}, those millimeters are our obsession."
                            </p>
